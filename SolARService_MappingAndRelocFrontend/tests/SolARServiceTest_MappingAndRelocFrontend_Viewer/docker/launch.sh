...
docker rm -f solarservicemappingandrelocfrontendviewer
docker run -it -d -e DISPLAY -e MAPUPDATE_SERVICE_URL -e MAPPINGANDRELOCFRONTEND_SERVICE_URL -e SOLAR_LOG_LEVEL -e "SERVICE_NAME=SolARServiceMappingAndRelocFrontendViewer" -v /tmp/.X11-unix:/tmp/.X11-unix --net=host --log-opt max-size=50m -e "SERVICE_TAGS=SolAR" --name solarservicemappingandrelocfrontendviewer artwin/solar/services/mappingandrelocfrontend-viewer:latest
